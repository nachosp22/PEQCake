...
class StoreOrderRequest extends FormRequest
{
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            $cakeIds = collect($pedido)
                ->pluck('cake_id')
                ->filter()
                ->map(static fn (mixed $cakeId): int => (int) $cakeId)
                ->filter(static fn (int $cakeId): bool => $cakeId > 0)
                ->unique()
                ->values();
            ...
            if (count(array_diff($cakeIds->all(), $availableCakeIds)) > 0) {
                $validator->errors()->add('pedido', 'Una de las tartas seleccionadas ya no está disponible.');
            }
        });
    }
}
